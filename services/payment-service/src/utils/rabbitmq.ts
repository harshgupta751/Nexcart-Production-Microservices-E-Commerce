...
class RabbitMQClient {
  ...
  async consume(queue: string, handler: (msg: object) => Promise<void>): Promise<void> {
    ...
    await channel.consume(queue, async (msg) => {
      if (!msg) return;
      try { const content = JSON.parse(msg.content.toString()); await handler(content); channel.ack(msg); }
      catch (err) { logger.error('Message handling failed', err as Error); channel.nack(msg, false, !msg.fields.redelivered); }
    });
