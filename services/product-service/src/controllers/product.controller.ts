...
import { successResponse, NotFoundError, createLogger, getPaginationParams, buildPagination, CACHE_KEYS, CACHE_TTL } from '@nexcart/shared';
...
export async function getProducts(req: Request, res: Response, next: NextFunction): Promise<void> {
  try {
    ...
    if (!search && !minPrice && !maxPrice) {
      ...
      if (cached) { logger.debug('Cache hit', { cacheKey }); res.json(JSON.parse(cached)); return; }
    }
...
export async function updateProduct(req: Request, res: Response, next: NextFunction): Promise<void> {
  try {
    const { id } = req.params;
    const product = await Product.findByIdAndUpdate(id, { ...req.body, updatedAt: new Date() }, { new: true, runValidators: true });
    if (!product) throw new NotFoundError('Product');
    await Promise.all([redisClient.del(CACHE_KEYS.PRODUCT(id)), invalidateProductListCache()]);
