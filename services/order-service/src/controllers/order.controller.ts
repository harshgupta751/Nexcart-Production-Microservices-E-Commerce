...
export async function createOrder(req: Request, res: Response, next: NextFunction): Promise<void> {
  try {
    ...
    if (!items || items.length === 0) {
      throw new ValidationError([{ message: 'Order must contain at least one item' }]);
    }
    ...
    clearUserCart(userId).catch((err) =>
      logger.warn('Failed to clear cart after order', { userId, err })
    );
...
export async function cancelOrder(req: Request, res: Response, next: NextFunction): Promise<void> {
  try {
    ...
    if (!['PENDING', 'CONFIRMED'].includes(order.status)) {
      throw new ValidationError([{ message: 'Order cannot be cancelled at this stage' }]);
    }
...
async function clearUserCart(userId: string): Promise<void> {
  ...
  await axios.delete(`${cartUrl}/api/cart`, {
